...
import { CATEGORIES, STATES, AP_DISTRICTS } from './cropData';
...
export default function FilterBar({ filter, setFilter, view, onBack, searchValue, setSearch, onSearchSelect }) {
  const cats = [['all', 'All']].concat(Object.entries(CATEGORIES).map(([k, v]) => [k, v.label]));
  const results = computeSearchResults(searchValue, view);
...
function computeSearchResults(query, view) {
  if (!query) return [];
  ...
  if (view.level === 'india') {
    for (const name of Object.keys(STATES)) {
      if (name.toLowerCase().includes(q) || STATES[name].code.toLowerCase() === q) {
    ...
    for (const name of Object.keys(AP_DISTRICTS)) {
      if (name.toLowerCase().includes(q)) {
        out.push({ kind: 'district', name, meta: AP_DISTRICTS[name].flagshipMaterial });
      }
    }
  }
  return out;
