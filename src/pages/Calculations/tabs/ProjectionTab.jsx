...
export default function ProjectionTab({ calc }) {
  const headers = ['Yr', 'Revenue', 'Var', 'Fixed', 'Op. Profit', 'Depr.', 'Interest', 'Subv.', 'EBT', 'Tax', 'PAT', 'Principal', 'Net Profit', 'NCF', 'Cum NCF', 'DSCR', 'Loan Bal.'];
  return (
      ...
      <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: "'JetBrains Mono', monospace", fontSize: 12 }}>
        ...
        <tbody>
          {calc.rows.map((row, i) => {
            ...
            const rowDscrColor = row.dscr === null ? C.fg2 : row.dscr >= 1.5 ? C.chartPositive : row.dscr >= 1.25 ? C.chartNeutral : C.chartNegative;
            return (
              <tr key={row.t} style={{ background: isBreakEven ? alpha(C.accent, 22) : i % 2 === 0 ? C.bg0 : C.bg1, borderBottom: `1px solid ${C.border}` }}>
                ...
                <td style={{ padding: '6px 9px', textAlign: 'right', color: row.subvention > 0 ? C.chartPositive : C.fg3 }}>{row.subvention > 0 ? fmtINR(row.subvention) : '—'}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: row.ebt < 0 ? C.chartNegative : C.fg1 }}>{fmtINR(row.ebt)}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: C.fg3 }}>{fmtINR(row.tax)}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: row.pat < 0 ? C.chartNegative : C.fg1 }}>{fmtINR(row.pat)}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: C.fg3 }}>{fmtINR(row.principal)}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: (row.netProfit ?? 0) < 0 ? C.chartNegative : C.chartPositive, fontWeight: 700 }}>{fmtINR(row.netProfit ?? 0)}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: row.ncf < 0 ? C.chartNegative : C.chartPositive, fontWeight: 700 }}>{fmtINR(row.ncf)}</td>
                <td style={{ padding: '6px 9px', textAlign: 'right', color: row.cumNCF < 0 ? C.chartNegative : C.chartPositive, fontWeight: 700 }}>{fmtINR(row.cumNCF)}</td>
