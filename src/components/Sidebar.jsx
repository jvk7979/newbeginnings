...
export default function Sidebar({ currentPage, onNavigate }) {
  ...
  return (
    <div style={{ width: 220, background: C.bg2, borderRight: `1px solid ${C.border}`, display: 'flex', flexDirection: 'column', flexShrink: 0, height: '100vh' }}>
      ...
      <div style={{ borderTop: `1px solid ${C.border}`, padding: '10px 8px' }}>
        <button style={{ display: 'flex', alignItems: 'center', gap: 9, fontFamily: "'DM Sans', sans-serif", fontSize: 13, color: C.fg3, padding: '7px 10px', borderRadius: 4, cursor: 'pointer', background: 'transparent', border: 'none', width: '100%' }}
          onMouseEnter={e => { e.currentTarget.style.background = C.bg3; e.currentTarget.style.color = C.fg2; }}
          onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = C.fg3; }}>
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" width="15" height="15"><circle cx="12" cy="12" r="3"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14M15.54 8.46a5 5 0 0 1 0 7.07"/></svg>
          Settings
        </button>
      </div>
    </div>
